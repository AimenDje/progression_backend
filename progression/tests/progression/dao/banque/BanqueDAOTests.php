...
use progression\dao\BanqueDAO;
use progression\domaine\entité\banque\{Banque, QuestionBanque};
...
final class BanqueDAOTests extends TestCase
{
	...
	{
		ChargeurFactoryBanque::set_instance(null);
		parent::tearDown();
	}

	public function test_étant_donné_un_uri_de_banque_inexistant_lorqsuon_la_charge_on_obtient_null()
	{
		$this->assertNull((new BanqueDAO())->get_banque("file://inexistant.yml"));
	}
	
 /*   
	public function test_étant_donné_un_fichier_de_banque_minimal_lorsquon_charge_la_banque_on_obtient_les_valeurs_par_défaut()
	{
		...
		$résultat_obtenu = (new BanqueDAO())->get_banque("file://" . __DIR__ . "/démo/défauts/info.yml");

		$this->assertEquals($résultat_attendu, $résultat_obtenu);
	}

*/
	public function test_étant_donné_un_fichier_de_banque_valide_lorsquon_charge_la_banque_on_obtient_un_objet_Banque_correspondant()
	{
		
		$banque = new Banque(
			nom: "contenu",
			url: "C:\Users\Anayees\Downloads\contenu.yml"
		);

		$question1 = new QuestionBanque(
			nom: "Question1",
			url: "https://git.dti.crosemont.quebec/asarkes/progression_questions-test/-/raw/main/QuestionsYAML/exemple-anayees.yml"
		);

		$question2 = new QuestionBanque(
			nom: "Question2",
			url: "https://git.dti.crosemont.quebec/asarkes/progression_questions-test/-/raw/main/QuestionsYAML/exemple-anayees-1.yml"
		);

		$banque->ajouterQuestionsBanque($question1);
		$banque->ajouterQuestionsBanque($question2);

		$questions = $banque->getQuestions();

		$this->assertContains($question1, $questions);
		$this->assertContains($question2, $questions);
		$this->assertCount(2, $questions); 

		$specificQuestion = $banque->getQuestionParNom("Question1");
		$this->assertEquals($question1, $specificQuestion);
	}
/*
